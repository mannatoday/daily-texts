...
| 觸發 | 說明 |
|------|------|
| `schedule`（cron） | 預設 `0 6 * * *` 與 `0 9 * * *`（UTC）＝台北 14:00／17:00；可直接改 YAML 內 cron |
...
- **排程標籤**：log／commit message 會標示 `schedule:0 6 * * *`、`schedule:0 9 * * *` 或 `manual`
...
| 觸發 | 說明 |
|------|------|
| `schedule` | `0 13 * * *`（UTC）＝台北 21:00，在 06:00／09:00 發布之後 |
