...
class HtmlFormatter:
    ...
    def format(
        ...
        *,
        ...
        if stylesheet_href:
            style_block = f'  <link rel="stylesheet" href="{escape(stylesheet_href, quote=True)}" />\n'
        else:
...
def _day_nav(
    *,
    ...
    def _link(href: str | None, label: str, css: str) -> str:
        if href:
            return f'    <a class="{css}" href="{escape(href, quote=True)}">{label}</a>\n'
    ...
    return (
        '  <nav class="day-nav" aria-label="日期導覽">\n'
